              ...
              <table id="example2" class="table table-bordered table-hover view-data">
                ...
                $this->load->model('Inquiry_model');
                  foreach ($arr as $info)
                  {
                    ...
                    $cnt = $this->db->get('web_followup')->num_rows();

                    $res = $this->Inquiry_model->web_last_followup($info['id']);
                    if($info['status']=='0')
                    {
                        $status = "Pending";
                        $color = "black";
                ...
                <tr style="color: <?php echo $color;?>">
                    ...
                    <td><?php echo $status; ?> <?php if(!empty($demo_details)){?><div style="border-top: solid 1px black;margin-top: 5px"><?php echo $demo_details['name']; }?></div></td>
                    <td><?php echo $res['followup_reason']?></td>
                    <td><?php echo $staff_info['name'];?></td>
                   
                    <td align="center">
                      <a href="<?php echo site_url('inquiry/web/'.$info['id']);?>"><div class="fas fa-edit"></div></a>
                      &nbsp<a href="<?php echo site_url('web_followup/view_followup/'.$info['id']);?>"><div class="fas fa-eye"></div></a>
                      <?php 
                      if($this->session->userdata('user_role')!=3){
                      ?>
                      &nbsp<a href="<?php echo site_url('/demolecture/add_demo_offline/website/'.$info['id']);?>"><div class="fas fa-clock"></div></a>
                      <?php }?>
