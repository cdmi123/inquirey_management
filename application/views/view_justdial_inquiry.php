          ...
          <div class="col-sm-12">
            <div class="row">
              ...
              <a href="<?php echo site_url('inquiry/view_inquiry');?>" class="btn btn-primary m-1 float-right" ><i class="fas fa-list"></i> Offline List</a>
               ...
               <a href="<?php echo site_url('inquiry/show_pending_inquiry');?>" class="btn btn-secondary m-1 float-right" ><i class="fas fa-list"></i> Pending Inquiry</a>

              <a href="<?php echo site_url('inquiry/show_admission_inquiry');?>" class="btn btn-success m-1 float-right" ><i class="fas fa-list"></i> Admission Inquiry</a>

              <a href="<?php echo site_url('inquiry/show_decliened_inquiry');?>" class="btn btn-danger m-1 float-right" ><i class="fas fa-list"></i> Decliened Inquiry</a>
              ...
              <table id="example2" class="table table-bordered table-hover view-data">
                ...
                $this->load->model('Inquiry_model');
                  foreach ($arr as $info)
                  {
                    ...
                    $cnt = $this->db->get('justdial_followup')->num_rows();

                    $res = $this->Inquiry_model->justdial_last_followup($info['id']);
                    if($info['status']=='0')
                    {
                        $status = "Pending";
                        $color = "black";
                    ...
                    $demo_details = $this->Inquiry_model->get_inquiry_demo($info['id'],'justdial');
                   // $color = "black";
                    //$status = "Pending";
                    //if($info['status']=='2'){
                      //$color = "green";
                ...
                <tr style="color: <?php echo $color;?>">
                    ...
                    <td><?php echo $status; ?> <?php if(!empty($demo_details)){?><div style="border-top: solid 1px black;margin-top: 5px"><?php echo $demo_details['name']; }?></div>
                      <br>
                      <a href="<?php echo site_url('inquiry/update_justdial_visited_status/'.$info['id']) ?>" class="btn btn-success">Visited</a> 

                      <a href="<?php echo site_url('inquiry/update_justdial_decliened_status/'.$info['id']) ?>" class="btn btn-danger">Declined</a></td>

                    <td><?php echo $res['followup_reason']?></td>
                    <td><?php echo $staff_info['name'];?></td>
                   
                    <td align="center">
                      <a href="<?php echo site_url('inquiry/justdial/'.$info['id']);?>"><div class="fas fa-edit"></div></a>
                      &nbsp<a href="<?php echo site_url('justdial_followup/view_followup/'.$info['id']);?>"><div class="fas fa-eye"></div></a>
                      <?php 
                      if($this->session->userdata('user_role')!=3){
                      ?>
                      &nbsp<a href="<?php echo site_url('/demolecture/add_demo_offline/justdial/'.$info['id']);?>"><div class="fas fa-clock"></div></a>
                      <?php }?>
